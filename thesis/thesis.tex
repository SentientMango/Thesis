...
The colliding protons are prepared for collisions by a sequence of systems, such as LINAC 2 (a liner accelerator), the Proton Synchrotron Booster (PSB), the Proton Synchrotron (PS), and the Super Proton Synchrotron (SPS), which progressively increase their energy. The highly energetic protons (having an energy of about 450 GeV each) are then injected into the LHC ring.
...
\caption{Overview of a proton-proton $pp$ collision. The initial partons inside the protons (the green ovals) undergo initial state radiation, and interact in the hard process (red circle). The hard interaction results in a shower of partons (red curves) that hadronise into color neutral states (light green circles). The proton remnants then participate in a secondary interaction (purple ellipse) creating another parton shower (in purple), which hadronises and decays into stable particles. This, along with the beam remnants (light blue ellipses), is part of the underlying event. Charged particles can emit electromagnetic radiation (yellow) at any point.}
\label{fig:pp}
\end{figure}
...
The rate at which a scattering process occurs is called the cross section $\sigma$ of the process. The cross section is expressed in barns, a unit of area, where $1 b = 10^{-24}cm^2$. The number of collisions is characterised by the luminosity $\mathcal{L}$. The luminosity is indicative of the performance of a particle accelerator, and is defined as the ratio of the rate of event detection to the cross section (Equation \ref{eq:lumi}):
\begin{equation}
\mathcal{L} = \frac{1}{\sigma}\frac{\text{d}N}{\text{d}t}
\label{eq:lumi}
\end{equation}
\myequations{Luminosity}
...
\end{equation}
\myequations{Rate of detection of events with a final state $X$}
For head-on colliding protons, each with momentum $P$, such that their center-of-mass energy is $\sqrt{s}=2P$, the interacting partons will have a total energy $\sqrt{\hat{s}}=\sqrt{2x_1x_2}P$, where $x_1$ and $x_2$ give the fraction of its proton momentum carried by each parton. For the process $pp\to X$, the cross section is calculated using Equation \ref{eq:cross_section}.
\begin{equation}
\sigma(pp\to X) = \sum_{n=0}^{\infty} \alpha_S^n(\mu_R^2) \sum_{i,j} \int\mathrm{d}x_1\mathrm{d}x_2 f_{i/p}\left(x_1,\mu_F^2\right)f_{j/p}\left(x_2,\mu_F^2\right)\times \hat{\sigma}_{ij\to X}^{(n)}\left(x_1x_2s,\mu_R^2,\mu_F^2\right)
\label{eq:cross_section}
\end{equation}
\myequations{Cross section $\sigma$ for the $pp\to X$ process}

where $f_{i/p}$ and $f_{j/p}$ are the parton distrbution functions describing the fraction of proton momentum carried by the $i^{th}$ and $j^{th}$ partons. Here, $n$ is the order of $\alpha_S$ at which the cross section is calculated. If $X=ZZ$, such as for the process $pp\to ZZ$, at LO ($n=0$) the hard (partonic) cross section $\hat{\sigma}_{ij\to ZZ}^{(n)}\left(x_1x_2s,\mu_R^2,\mu_F^2\right)$ is proportional to $\delta(x_1x_2s - M_Z^2)$. Thus, this term non zero only for partons $i,j$ if they can produce a $Z$ boson, such as $i=q$ and $j=\bar{q}$. For $n\geq 1$, partonic channels such as $gq$ and $gg$ also contribute, and there is no restriction for $x_1x_2s = M_Z^2$. Equation \ref{eq:cross_section} also illustrates the dependence of the cross section on renormalization and factorization scales, $\mu_R$ and $\mu_F$ respectively. The separation of the calculation into perturbative hard scattering physics and non-perturbative Parton Distribution Functions greatly simplifies QCD calculations.
...
\end{equation}
\myequations{Rapidity $y$}
in turn, is an effective coordinate due to its Lorentz invariance under boosts in the $z$-direction.

The separation between two objects in the detector is described by 
\begin{equation}
\Delta R = \sqrt{\Delta\eta^2 + \Delta\phi^2}
\end{equation}
\myequations{Separation $\Delta R$ between two objects}
...
Events in a given process are generated using Monte Carlo methods, where points in the kinematic phase-space are chosen using random sampling. These are used in Monte Carlo generator software such as MadGraph\cite{MadGraph} to calculate the matrix element of the process. Such generators use parton distribution functions (PDFs) to model the interaction between partons up to a given order in QCD, with higher order corrections being accounted for by a ``k-factor''. The radiating partons after the hard interaction result in a shower, which are modelled by parton showering software, such as Pythia\cite{pythia}. The output of these steps is input into the next step of simulation, and is also used for generator-level studies, called ``truth-level''.

In the next step, called detector simulation, the particles are propagated through the various layers of the detector, where the effects of the detector material, and it interaction with the particles, are accounted for in the simulation. The goal is to have a resulting data set identical to the one obtained from the physical detector. Software such as Geant4\cite{geant4}. This is a slow process, and often a faster but more approximate detector simulation is used. Detector simulation digitizes the interaction of particles by emulating the response of the electronics in the detector.

The data after the simulation of the detector is used to identify and reconstruct objects as particles, such as muons, electrons, photons or jets. Energy deposits that are not identified and matched to physical objects are collected together as ``soft-terms'' that are used to calculate the missing transverse energy \met.

\subsection{Missing Transverse Momentum}

Stable particles that do not interact electromagnetically or hadronically pass through the various detector layers without interacting, and are thus not visible to the detector. Collisions between the partons (which carry some fraction of the proton's momentum) take place along the beam axis ($z$-axis). Along the $x$- and $y$-axes, i.e. in the transverse place, the momentum before the collision in zero. Thus such invisible objects are identified by an imbalance in the total momentum in the transverse plane. Thus, the missing transverse momentum, defined as the negative vector sum of the transverse momenta of all visible reconstructed objects in the event,
\begin{equation}
\vec{p}_T^\textrm{miss} = -\sum_{visible}\vec{p}_T
\end{equation}
where visible objects include muons, electrons, photons, taus, jets, and the soft-terms that do not correspond to physical objects.

For predicted particles with high mass, or particles that carry high momentum away from the hard interaction vertex, a large imbalance is expected.

The magnitude of the $\vec{p}_T^\textrm{miss}$ is denoted by \met.
...
\hline
\end{tabular}
\caption{Event selection criteria in the $\ell\ell+$\met search as shown in ATLAS ZH search \cite{ZH_ATLAS}}
...
It is possible to estimate the \ZZ using $ZZ\to \ell\ell\ell\ell$ data. However, the precision of this method would be severely limited by the small number of $ZZ\to\ell\ell\ell\ell$ events in data. The branching fraction $Z\to\ell\ell$ for one flavor of lepton ($e/\mu$) is $\approx 3.4\%$, and $Z\to\nu\nu$ is 20\%. 
\begin{align}
BR(ZZ\to \ell\ell\ell\ell) &= (2\times 0.034)\times(2*0.034) = 0.00462\\
BR(ZZ\to \ell\ell\nu\nu) &= (2\times 0.034)\times(0.2)\times 2 = 0.0272
\end{align}
\myequations{Branching ratios of $ZZ$ final states}
Thus, branching fraction of $ZZ\to \ell\ell\ell\ell$($\approx$0.46\%) compared to \ZZ (2.7\%) is smaller by a factor of 6. The low branching fraction of $ZZ\to \ell\ell\ell\ell$ limits the precision.  The statistics are further limited by detector effects.
...
Figure \ref{fig:Zg}.d shows the Final State Radiation (FSR) diagram for the $\ell\ell\gamma$ final state. The photon is in this case radiated off a final state lepton. This process must be suppressed as it does not correspond to a similar \ZZ process.  Imposing a mass window requirement on the reconstructed $Z$ boson candidate mass suppresses this diagram.
...
\section{Approach}
This thesis plans to estimate the $ZZ$ background contribution to the $ll+$\met final state using $Z(\to ll)\gamma$ data, where the photon models the Standard Model invisible $Z$ boson. A transfer factor $R$ is introduced as the ratio of the cross sections of \ZZ to \Zg. In the high $Z$ boson $p_T$ region, the two processes are kinematically similar, therefore the curve of the transfer factor $R$ as a function of $p_T$ is expected to approach a constant value. This transfer can be used to estimate to the contribution of \ZZ from \Zg data.

A ratio of the cross sections of \ZZ and \Zg processes is taken to obtain the $R$ distribution as a function of \met, or $p_T(\gamma)$. The uncertainty on $R$ is calculated by estimating the possible effect of missing higher order corrections, the PDF sets used, photon fragmentation, etc. The contributions of the $q \bar{q}$ and $gg$ processes are estimated separately.
...
\end{equation}
\myequations{Definition of the transfer factor $R$}
...
\end{equation}
\myequations{Couplings of the $Z$ boson and photon to quarks}
...
\label{eq:theory_R}
\end{equation}
\myequations{Cross section approximations for $q\bar{q}\to ZZ$ and $Z\gamma$}
...
\end{equation}
\myequations{Theoretical approximation of the transfer factor $R$}
...
The infrared divergences are addressed by the inclusion of the factorization scale $\mu_F$, while the ultraviolet divergences are addressed by the inclusion of the renormalization scale $\mu_R$. These parameters are arbitrary, and are set by hand. These are then varied between $\frac{1}{2}\mu < \mu < \ 2\mu$ to obtain an indication of the dependence of the matrix element on the scales. 

A perfect calculation would include an infinite number of terms in $\alpha_S$, and would result in the $\mu$-dependent terms cancelling out perfectly, resulting in an answer that is independent of the choice of $\mu$. However, perturbative QCD calculations get progressively more computationally expensive as the order of the perturbative theory increases. Thus, perturbative QCD calculations are only carried out up to a fixed order. In the absence of a perfect infinite order calculation, the variation of the factorization and renormalization scales gives an estimate of missing higher order contributions.

Varying the scales is not a complete treatment of the higher order QCD effects. In addition to it, the process dependent part of the missing higher order contributions will be treated using $K$-factors. These $K$-factors account for the corrections between the leading order and higher order calculations, and the difference between the $K$-factors of the two process will give a handle on the degree of correlation between the two processes.

\section{Photon Isolation}\label{sec:Isolation}
The \Zg process may contain photons that arise from the hadron showers. Photons that are produced in the hard interaction are well isolated from activity. However, photons may also be produced from the fragmentation of partons as part of a hadronic jet. A clean sample of well isolated photons is important for efficient background rejection, and in this case, to obtain a clear picture of \Zg events.

Experimentally, photon isolation is implemented with the following selection:
\begin{equation}
\sum_{\in R_0} E_T(\text{had}) < \epsilon_h p_T^\gamma
\label{eq:photon_isol}
\end{equation}
\myequations{Experimental definition of photon isolation}
limiting the transverse hadronic energy $E_T(had)$ in a cone of size $R_0 = \sqrt{\Delta\eta^2 + \Delta\phi^2}$ around the photon, to some fraction of the photon $p_T$. This rejects events where the photon is associated with any hadronic activity in the cone. While this method works well for prompt photons, for non-perturbative fragmentation, this method of isolation constrains the phase space of events with soft gluons, which hinders the cancellation of infrared divergences. This method is currently used in experimental analyses at ATLAS.

The smooth cone isolation method of Frixione \cite{frixione} is an alternative isolation procedure, which simplifies calculations by avoiding photon-to-hadron fragmentation contributions, treats both gluons and quarks in the same way. The Frixione method of isolation can be applied to experimental data in hadron-hadron, photon-hadron and electron-positron collisions as well. In the Frixione method, the following isolation prescription is applied to the photon:
\begin{equation}
	\sum_{R_{j\gamma} \in R_0} E_T(\text{had}) < \epsilon_h p_T^\gamma \left(\frac{1-\cos R_{j\gamma}}{1-\cos R_0}\right)^n.
\label{eq:frix_isol}
\end{equation}
\myequations{Frixione (smooth-cone) method of photon isolation}
where $R_{j\gamma}$ is the separation of the photon and the $j^{th}$ parton. This constrains the total hadronic energy inside a cone of radius $R_{j\gamma}$, for all separations $R_{j\gamma}$ less than a chosen cone size $R_0$. For $n=0$, Equation \ref{eq:frix_isol} is equal to Equation \ref{eq:photon_isol}, but for $n \geq 1$, this prescription allows soft radiation inside the photon cone and collinear singularities are removed. The inclusion of soft radiation makes the Frixione isolation infrared finite at all orders, can be applied to events having non-perturbative photon fragmentation, and avoids collinear singularities, that occur when a parton is collinear with the photon, by requiring it to be soft.

\chapter{Transfer factor R and the uncertainties associated with it}\label{ch:Results}

\section{MCFM}\label{sec:MCFM}
Monte Carlo for FeMtobarn processes (MCFM) is a program that calculates cross sections for femtobarn-level processes at leading order(LO) or next to leading order (NLO) in QCD. NNLO predictions in QCD, and NLO electroweak corrections are also provided for some processes. In this study, MCFM v8.0 \cite{MCFM1, MCFM2, MCFM3, MCFM} is used to generate cross sections of \ZZ and \Zg processes at NLO, with a selection of generator level cuts. The generation parameters in MCFM allow fine control over the sample, such as PDF sets, photon isolation, lepton and photon $p_T$ and $\eta$, renormalization and factorization scales, etc. The samples are generated with cuts on $E_T^{miss} = p_T(Z\to \nu\nu)$ for the $ZZ$ process and $p_T(\gamma)$ for the $Z+\gamma$ process. Table \ref{table:default} lists the generator level settings used for the $ZZ$ and $Z+\gamma$ processes. All lepton cuts are consistent with the ones used in the ATLAS Z+$E_T^{miss}$ analysis \cite{ZH_ATLAS}, as shown in Table \ref{table:event_selection}.
...
\end{equation}
\myequations{The inclusive transfer factor $R$ corrected for the branching fractions of the $Z$ bosons}
...
\label{eq:scale}
\end{equation}
\myequations{Six point variation of renormalization and factorization scales}
...
\end{align}
\myequations{Central cross section value, and scale uncertainty width}
where
\begin{align}
...
\end{align}
\myequations{Maximum and minimum cross sections for evaluating scale uncertainties}
...
\begin{equation}
K_{NLO}^{(V)} = \frac{\sigma_{NLO}^{(V)}}{\sigma_{LO}^{(V)}}
\label{eq:K_factor}
\end{equation}
\myequations{Definition of the NLO $K$-factor}
As seen in Figure \ref{fig:Rcurve}, the LO and NLO predictions for the transfer factor $R$ agree quite well at high $p_T$, indicating that the $K$-factors for the \ZZ and \Zg processes behave similarly in that regime, and that higher order QCD corrections are highly correlated between the two processes at high $p_T$. The difference between the $K$-factors of the two processes will be a measure of the difference in the contribution of higher order QCD corrections to them. This difference, $\delta K_{NLO}$, is taken as the $K$-factor uncertainty.

The following assumption is made regarding the NLO $K$-factor.
\begin{equation}
\frac{\sigma^{(V)}_{NLO}}{\sigma^{(V)}_{LO}} >\frac{\sigma^{(V)}_{N^{\infty}LO}}{\sigma^{(V)}_{NLO}}
\end{equation}
\myequations{Assumptions regarding $K$-factor differences at higher orders}
It is also assumed that the $K$-factor difference at NLO is less than the $K$-factor difference at higher orders.
\begin{equation}
...
Figure \ref{fig:K_pt} shows the $K$-factor distributions for both processes, as well as the $K$-factor uncertainty $\delta K_{NLO} = K_{NLO}^{(\gamma)}(p_T) - K_{NLO}^{(Z)}(p_T)$. The $K$-factor grows larger as a function of $p_T(V)$ for both processes. However, the difference between the two $K$-factors shrinks, due to the two processes being similar at high $p_T$. 

\begin{figure}[H]
\centering
	\includegraphics[width=0.7\textwidth]{K_pt.png}
	\caption{The $K$ factor to estimate the unknown process dependent correlations, defined as $\sigma_{NLO}(V)/\sigma_{LO}(V)$. The bottom plot shows the difference between the $ZZ$ and $Z\gamma$ $K$-factors, $\delta K_{NLO}$, relative to $K(Z)$.}
	\label{fig:K_pt}
\end{figure}
Thus, the $K$-factor uncertainty $\delta K_{NLO}$, is 4\% at $p_T(V) = 500$ GeV. At low $p_T$ the $K$-factors differ significantly between the two processes, e.g. at 100 GeV the difference is 23.6\%
...
\end{equation}
\myequations{Parton Distribution Function uncertainties}
...
\end{split}
\end{equation}
\myequations{Photon Fragmentation uncertainties}
