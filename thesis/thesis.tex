...
Thus, there are 8 gluons, which are massless spin 1 particles with an intrinsic property called color, that mediate strong interactions (described by Quantum Chromodynamics). They are responsible for interactions between quarks (leptons do not interact via the strong force). At low energies, quarks cannot be found in isolation, instead occurring in triplets called $Baryons$, or a bound quark-antiquark pair, called $Mesons$. This is because of color confinement. 
...
The Standard Model is based on the formalism of quantum field theories. 
...
\begin{figure}[H]
	\centering
	\includegraphics[width= 0.8\textwidth]{Ratio_default.png}
...
\end{figure}
The $R_{gg}$ distribution is observed to approach an asymptotic value at a much higher $p_T = 1.5$ TeV. The shape and scale of the $R_{gg}$ distribution (Figure  remain to be understood, as they differ from Figure \ref{fig:Rcurve}.
