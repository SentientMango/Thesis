...
\item Matter-antimatter asymmetry: In the Big Bang, matter and antimatter should be created in equal quantities. However, the Universe appears to be almost completely made out of matter, indicating that in its initial state of the Universe, this symmetry was broken\cite{Baryon_Asymmetry}.
...
\item Gravity: The Standard Model does not include gravity. In fact, the Standard Model is incompatible with general relativity\cite{grav_inc}.
...
Photons in the first mechanism are prompt and well isolated from final state hadrons, whereas in the second mechanism, the photons are part of a hadronic jet. A clean sample of well isolated photons is important to obtain efficient background rejection, and in this case, to obtain a clear picture of \Zg events.
...
Varying the scales does not provide a complete picture of the missing higher order contributions. In going from LO to NLO, a significant difference is observed in the cross section prediction for both processes. However, if both processes are similar at LO, they are expected to be similar at all higher orders as well. To account for process dependent effects of QCD corrections at higher orders, we use the NLO $K$-factor, defined as the ratio of the cross section at NLO to the cross section at LO, as shown in equation \ref{eq:K_factor}.
...
It is assumed that the difference between the NLO and LO cross sections is greater than the difference between the perfect prediction and the NLO cross section, i.e. the NLO $K$-factor is larger than higher order $K$-factors.
\begin{equation}
