...
\newcommand{\llM}{$\ell\ell+E_T^{miss}$ }
...
\hline
$\Delta\phi(\vec{p}_T^{\ell\ell},\vec{E}_T^{miss})$ & $\Delta\phi(\vec{p}_T^{\ell\ell},\vec{E}_T^{miss})>2.7$ radians\\
\hline
...
\bibitem{bunch}
\textbf{Rende Steerenberg, Stefania Pandolfi}, LHC report: full house for the LHC, \url{http://cds.cern.ch/record/2272573}
